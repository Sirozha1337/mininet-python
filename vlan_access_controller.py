...
class Tutorial (object):
  ...
  def act_like_switch (self, packet, packet_in):
    ...
    if packet.dst in self.mac_to_port: 
      ...
      if self.vlan_to_port[packet_in.in_port] == self.vlan_to_port[self.mac_to_port[packet.dst]]:
      ...
      # Sending to all ports in same vlan as the input port
      log.debug("DPID: %s" % self.connection.dpid)
      log.debug("Input port: %s" % packet_in.in_port )
      for port in self.connection.ports:
        log.debug("Checking port: %s" % port )
        if port != packet_in.in_port and port != 65534 and self.vlan_to_port[port] == self.vlan_to_port[packet_in.in_port]:
